...
This is the Ponceletian family of excentral triangles to billiard 3-periodics. If the axes of its caustic are $a,b$, this family is inscribed in an ellipse with $a_e,b_e$ given in \cref{thm:03-incenter-excenter}; see \cref{fig:03-n3-affine}(left).
...
\noindent \textbf{Affine image:} As \cref{fig:03-n3-affine}(middle) depicts, the incircle family can also be obtained from an affine image of billiard 3-periodics which sends the confocal caustic to a circle. Let $\alpha,\beta$ be the semi-axes of its billiard ellipse pre-image.
...
The homothetic family, shown in \cref{fig:03-homoth-brocard}, is the Poncelet family in a CAP pair for which the outer and inner ellipse are homothetic to each other, i.e., $a = k a_c$ and $b = k b_c$, where $a,b$ and $a_c,b_c$ are outer and inner ellipse semi-axes, respectively. Cayley imposes a homothecy ratio $k=1/2$.
...
\subsection{Vertex Parametrization for Generic CAP pair}
...
\begin{align*}
    P_2& =[x_2,y_2]=\frac{1}{k_2}\left[\frac{u_1 x_1 + u_2 y_1}{b} ,  \frac{w_1 x_1 + w_2 y_1}{a} \right] \\
...
We can obtain parametrizations for specific families by setting the caustic semi-axes $a_c$, $b_c$ as follows:
...
\hline
\makecell[lc]{conf.\\excentrals} &  & & \cref{thm:03-incenter-excenter} \\
\hline
\end{tabular}
