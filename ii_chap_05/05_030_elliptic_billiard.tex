 \begin{figure}
	...
	%	\input{pics_tex/caustica1.pdf_tex}
	 \includegraphics[scale=0.55]{pics-05-3tipos-orbitas.pdf}
...
\begin{figure}
	...
	%	\input{pics_tex/bilharorbita.pdf_tex}
		 \includegraphics[scale=0.45]{pics-05-050-orbitas-dinamica.pdf}
...
\begin{figure}
    \centering
    \includegraphics[scale=0.5]{pics-05-0010-bryant_poncelet_pair.pdf}
...
% \begin{remark} The number  $\rho$ is the rotation number of the diffeomorphism $f=\tau\circ\sigma$.   Considering an orbit $p_0, p_1, p_2,\ldots, $ it follows that
% \[ \rho(f)= \frac{1}{L} \lim_{n\infty}\left(\frac{1}{n} \sum_{i=0}^{n}\int_{p_i}^{p_{i+1}} \varphi(p) d\theta \right) \]


% \end{remark}
  %See \cite{franks-2003} 
 %\begin{theorem}
 %Let $f : \mathbb{T}^1\to\mathbb{T}^1 $ be an orientation preserving homeomorphism
%which preserves a probability measure $d\mu$, and let $F:\mathbb R\to\mathbb R$ be a lift of $f$ and suppose $\varphi:\mathbb{T}^1\to\mathbb R$ 
 % is the displacement function of $F$. Then
 % \[\rho(f)=\int_{\mathbb T^1} \varphi d\mu\]
 %\end{theorem}
...
\begin{corollary} 
In   above conditions, 
%let $\rho=\alpha/L$ the rotation number of $\tau\circ\sigma$. Then 
